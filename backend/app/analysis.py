...
def _clip01(value: float) -> float:
    return _safe_float(float(np.clip(value, 0.0, 1.0)))
...
def _rms_energy(y: np.ndarray, hop_length: int) -> tuple[np.ndarray, list[float]]:
    ...
    return normalized, _bucket(normalized, 64)


def _spectral_flux(y: np.ndarray, sr: int, hop_length: int) -> np.ndarray:
...
def _phrase_boundaries(beats: np.ndarray, phrase_beats: int = 32) -> list[float]:
    if beats.size == 0:
        return []
    return [_safe_float(beats[index]) for index in range(0, len(beats), phrase_beats)]
...
def _first_major_increase(times: np.ndarray, signal: np.ndarray, beats: np.ndarray, duration: float, start_ratio: float, end_ratio: float) -> tuple[float, float]:
    ...
    smoothed = np.convolve(signal, np.ones(8) / 8, mode="same")
...
def _low_complexity_region(times: np.ndarray, energy: np.ndarray, novelty: np.ndarray, duration: float, start_ratio: float, end_ratio: float) -> tuple[float, float]:
    ...
    for idx in range(start, max(start + 1, end - window)):
        ...
        novelty_slice = novelty[idx:min(len(novelty), idx + window)] if novelty.size else np.array([0.0])
...
def analyze_audio_file(path: Path) -> AnalysisResult:
    ...
    novelty = (onset_norm[:min_len] * 0.55 + flux[:min_len] * 0.45) if min_len else onset_norm
    ...
    intro_end, intro_conf = _first_major_increase(frame_times, energy + np.pad(novelty, (0, max(0, len(energy) - len(novelty))))[:len(energy)], beat_times, duration, 0.04, 0.28)
    vocal_start = _nearest(beat_times, max(intro_end, duration * 0.12))
    drop, drop_conf = _first_major_increase(frame_times, energy + np.pad(novelty, (0, max(0, len(energy) - len(novelty))))[:len(energy)], beat_times, duration, 0.15, 0.62)
    ...
    structure = {
        ...
        "phrase_boundaries": phrases[:128],
        "novelty_curve": _bucket(novelty, 64),
        "onset_strength": _bucket(onset_norm, 64),
        "spectral_flux": _bucket(flux, 64),
    }

    return AnalysisResult(
        estimated_bpm=_safe_float(estimated_bpm),
        duration=_safe_float(duration),
        beat_timestamps=[_safe_float(value) for value in beat_times[:1024]],
        downbeat_timestamps=[_safe_float(value) for value in downbeats[:256]],
        phrase_boundaries=phrases[:128],
